...
data_dir = pathlib.Path("../anh_train/Anh_em/train")  # biến đường dẫn đào tạo
...
loaded_model_11 = tf.keras.models.load_model("saved_trained_model_anh_em.h5")
# print(loaded_model_11.evaluate(valid_data))
# Tải  hình ảnh

def predict_1_class(img):
    pred = loaded_model_11.predict(tf.expand_dims(img, axis=0))
    pred_class = class_names[int(tf.round(pred)[0][0])]
    return pred_class

def predict_multi_class(img):
    pred = loaded_model_11.predict(tf.expand_dims(img, axis=0))
